...
export default function RentHistory() {
  ...
  useEffect(() => {
    import("./history.json").then((mod) => setHistory(mod.default || []));
  }, []);

  // Extract unique months and years from data
  const months = useMemo(() => getUnique(history, "rentMonth").map(m => m.split(" ")[0]), [history]);
  const years = useMemo(() => getUnique(history, "rentMonth").map(m => m.split(" ")[1]), [history]);
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="text-2xl font-bold text-black mb-6">Rent History</div>
      {/* Filters */}
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-100">
            <tr>
              ...
              <th className="px-4 py-2 text-left text-xs font-semibold text-black uppercase">Due (After Paid)</th>
              <th className="px-4 py-2 text-left text-xs font-semibold text-black uppercase">Advance Rent (Due)</th>
            ...
            {paginatedHistory.length === 0 ? (
              ...
              paginatedHistory.map((record) => (
                <tr key={record.id} className="hover:bg-gray-50">
                  <td className="px-4 py-2 text-sm text-black">{record.name}</td>
                  <td className="px-4 py-2 text-sm text-black">{initialCategories.find((c) => c.id === record.category)?.title || "-"}</td>
                  <td className="px-4 py-2 text-sm text-black">₹{record.paidAmount}</td>
                  <td className="px-4 py-2 text-sm text-black">₹{record.due}</td>
                  <td className="px-4 py-2 text-sm text-black">₹{record.advanceDue}</td>
                  ...
                  <td className="px-4 py-2 text-sm text-black">{record.rentMonth}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
          <div className="bg-white rounded-lg shadow-lg w-full max-w-md p-6 relative animate-fadeIn">
            ...
            <div className="mb-4 text-black space-y-2">
              <div><span className="font-semibold">Name:</span> {modal.record?.name}</div>
              <div><span className="font-semibold">Category:</span> {initialCategories.find((c) => c.id === modal.record?.category)?.title || "-"}</div>
              <div><span className="font-semibold">Paid Amount:</span> ₹{modal.record?.paidAmount}</div>
              <div><span className="font-semibold">Due (After Paid):</span> ₹{modal.record?.due}</div>
              <div><span className="font-semibold">Advance Rent (Due):</span> ₹{modal.record?.advanceDue}</div>
              ...
              <div><span className="font-semibold">Rent Month:</span> {modal.record?.rentMonth}</div>
